<script lang="ts">
	...
	const isDesktop = mediaQuery('(min-width: 1027px)');
	$: if ($navigating) {
...
<aside
	class="fixed left-0 top-[4rem] z-30 flex h-[calc(100vh-4rem)] min-w-[16rem] max-w-[16rem] shrink-0 -translate-x-[200%] transform flex-col border-r border-neutral-200 bg-white antialiased transition-[width,transform] duration-[500ms] ease-in data-[open=true]:translate-x-0 data-[open=false]:duration-[1000ms] lg:sticky lg:-ml-4 lg:block lg:translate-x-0 lg:duration-300 dark:border-neutral-900 dark:bg-neutral-950"
>
	<nav
